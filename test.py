...
def inside(name):
    global donedollar, bluenum, treenum, gun, fishingrod, keyone, apple, blueberries, fivedollar, jewelthree, bra , prettyflower, fish, magicpotion, jewelone, loveletter
    ...
    while runningins:
        ...
        if name == "wizarddoor":
            ...
            if ber.collidepoint(click) and bluenum >= 1:
...
def blitty():
    ...
    screen.blit(sprites.get("fishspot"), camera + (2000, 1300))
...
while running:
    ...
    pygame.draw.rect(screen, color, fr)
    ...
    cant_swim = pygame.sprite.collide_mask(Riva(sm_river, river_mask, rr), Playa(player, player_mask, fr))
